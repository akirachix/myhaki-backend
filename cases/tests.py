...
from .models import Case, Detainee, CaseAssignment
...
class CaseModelTest(TestCase):
    ...
    @patch('cases.serializers.CaseSerializer.translate_text')
    @patch('cases.serializers.CaseSerializer.geocode_location')
    def test_case_auto_assignment(self, mock_geocode, mock_translate):
        mock_translate.return_value = "alleged robbery at kiserian"
        mock_geocode.return_value = (-1.286389, 36.817223)

        case_data = {
            'detainee': self.detainee.detainee_id,
            'case_description': 'alleged robbery at kiserian',
            'trial_date': '2025-10-01',
            'police_station': 'kiserian police station',
            'stage': 'in_progress',
            'status': 'pending',
            'income_source': 'informal',
            'monthly_income': 'less_than_30000'
        ...
        assign_case_automatically(case.case_id)

        case.refresh_from_db()
        self.assertEqual(case.status, 'accepted')
        
        assignment = CaseAssignment.objects.filter(case=case).first()
        self.assertIsNotNone(assignment)
        self.assertEqual(assignment.lawyer, self.lawyer)